...
_DOWNLOAD_DIR = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "..", "data", "download-cache"
)
...
def _get_maf_mnist():
    ...
    splits_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "maf_mnist_splits.txt.gz"
    )
    ...
    return _data_arr_to_dict(
        [_preprocess_mnist(X[ind, :], y[ind], rng) for ind in ind_arr]
    )
...
def _get_maf_cifar10():
    ...
    labels = []
    for i in range(1, 6):
        data, labels = _get_cifar10_data_and_labels(
            os.path.join(path, "data_batch_" + str(i))
        )
        x.append(data)
    ...
    return _data_arr_to_dict(
        [
            ...
            for maf, flip in zip(
                (maf_train, maf_validation, maf_test), (True, False, False)
            )
        ]
    )
...
def _get_mnist_raw():
    def _download_from_other_source():
        ...
        warnings.warn(
            "Could not download from mldata.org, attempting "
    ...
    _make_dir(_DOWNLOAD_DIR)
    n_attempts = 3
    print("Attempting to load/fetch MNIST data via sklearn.datasets.fetch_openml")
    data_obj = None
        ...
        try:
            ...
            if i == n_attempts - 1:
                warnings.warn(
                    ...
                    "and failed each time, please check internet connection"
                    % n_attempts
                )
                raise
        else:
...
def _check_maf_data():
    ...
    for _data_name in ["mnist", "cifar10"]:
        ...
        for i, (x_direct, x_original) in enumerate(
            ...
            assert x_direct.dtype == x_original.dtype, (
                "dtypes not equal for index %d" % i
            )
...
def _get_maf_original(data_name):
    ...
    if sys.version_info < (3,):
        ...
        maf_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "..", "..", "maf"
        )
...
def _save_mnist_recreation_indices():
    ...
    train_idx, val_idx, test_idx = (
        mnist_ind[
            rev_maf_ind[
                np.sum(n_maf[:i], dtype=np.int) : np.sum(n_maf[: (i + 1)], dtype=np.int)
            ]
        ]
        for i in range(3)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "data_names",
        default=",".join(all_data_names),
        help="One or more data names separated by commas from the list %s"
        % str(all_data_names),
    )
