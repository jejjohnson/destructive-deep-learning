...
def make_toy_data(data_name, n_samples=1000, random_state=None, **maker_kwargs):
    ...
    X, y, is_canonical_domain = maker(
        n_samples, random_state=random_state, **maker_kwargs
    )
...
def _make_autoregressive(
    ...
    if x_distribution == "gaussian":
        ...
        raise ValueError(
            'x_distribution should be "gaussian", "uniform", or "abs-gaussian"'
        )
...
def _make_sin_wave(n_samples, x_scale=2 * np.pi, y_std=0.2, **kwargs):
    ...
    return _make_autoregressive(
        n_samples, func=np.sin, x_scale=x_scale, y_std=y_std, **kwargs
    )
...
def _make_rbig_sin_wave(n_samples, random_state=0):
    ...
    return _make_sin_wave(
        n_samples,
        x_scale=2,
        y_std=0.25,
        x_distribution="abs-gaussian",
...
def _make_grid(
    ...
    n_per_component = rng.multinomial(
        n_samples, 1 / n_components * np.ones(n_components)
    )

    X = np.vstack(
        [sample(pos, n, n_features) for pos, n in zip(positions, n_per_component)]
    )
...
def _make_concentric_circles(n_samples, n_circles=4, noise_std=0.1, random_state=0):
    ...
    circum = np.array([2 * np.pi * rad for rad in radius])
    perc_per_circle = circum / np.sum(circum)

    n_per_component = rng.multinomial(n_samples, perc_per_circle)

    theta = [2 * np.pi * rng.rand(n) for n in n_per_component]
    X = np.vstack(
        [
            (
                (r + noise_std * rng.randn(len(th)))
...
def _get_y_and_shuffle(X, n_per_component):
    y = np.concatenate([j * np.ones(n) for j, n in enumerate(n_per_component)])
    # Shuffle
